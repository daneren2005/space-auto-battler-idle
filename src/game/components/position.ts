...
export const positionDefinition: ComponentDefinition<PositionComponent, Float32Array, PositionConfig, PositionSerialization> = {
	...
	load(entity, memory, config) {
		...
		return {
			index,
			get x() {
				return block[POSITION_X];
			},
			set x(value: number) {
				block[POSITION_X] = value;
			},
			get y() {
				return block[POSITION_Y];
			},
			set y(value: number) {
				block[POSITION_Y] = value;
			},
			get width() {
				return block[POSITION_WIDTH];
			},
			set width(value: number) {
				block[POSITION_WIDTH] = value;
			},
			get height() {
				return block[POSITION_HEIGHT];
			},
			set height(value: number) {
				block[POSITION_HEIGHT] = value;
			},
			get angle() {
				return block[POSITION_ANGLE];
			},
			set angle(value: number) {
				block[POSITION_ANGLE] = value;
			},
		};
	},
	save(component) {
